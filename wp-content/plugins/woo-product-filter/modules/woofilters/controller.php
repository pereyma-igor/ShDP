...
class WoofiltersControllerWpf extends ControllerWpf {
	...
	public function filtersFrontend() {
		...
		if ( $recountPrice ) {
			$recountArgs = $this->createArgsForFilteringBySettings($settings, $queryvars, $filterSettings,
				$generalSettings, $recountPrice);
			if ( $showProducts || empty($categoryHtml) ) {
				$recountArgs['fields'] = 'ids';
				$loopRecount = new WP_Query($recountArgs);
				if ($loopRecount->posts) {
					foreach ($loopRecount->posts as $id) {
						if ($recountPrice) {
							$product = wc_get_product($id);
							if( $product->is_type('variable') ) {
								$priceMin = $product->get_variation_price();
								$priceMax = $product->get_variation_price('max');
								if ($priceMin && $priceMin < $prices['min_price']) {
									$prices['min_price'] = $priceMin;
								}
								if ($priceMax && $priceMax > $prices['max_price']) {
									$prices['max_price'] = $priceMax;
								}
							} else {
								$price = $product->get_price();
								if ($price && $price < $prices['min_price']) {
									$prices['min_price'] = $price;
								}
								if ($price && $price > $prices['max_price']) {
				...
				if ( $recountPrice && $prices['max_price'] ) {
	...
	public function createArgsForFilteringBySettings( $settings, $queryvars, $filterSettings = array(), $generalSettings = array(), $forRecountPrices = false ) {
			...
			if (!empty($setting['settings'])) {
				...
				switch ($setting['id']) {
					case 'wpfPrice':
						...
						if ( ( false !== $priceVal[0] ) && $priceVal[1] && !$forRecountPrices ) {
					...
					case 'wpfPriceRange':
						$priceStr = $setting['settings'][0];
						$priceVal = explode(',', $priceStr);
						if ( count($priceVal) == 2 && ( false !== $priceVal[0] ) && !$forRecountPrices ) {
